...
\subsection{Infrared Data}
     ...
     \caption{Ionization fraction of PAHs vs. band ratios of IRAS12 and 25, and WISE 12 and 25 micron bads vs. the AKARI 9 micron band, for three ISRF strengths: Top: $G_{0} = 100$, Middle: $G_{0} = 1000$, and Bottom: $G_{0} = 10000$. These ratios are determined by assuming the SED template if \cite{compiegne11} }
     \end{figure*}
...
\subsection{All-sky Pixel Domain Analysis}
    ...
    We can quickly see that, for all of the bands, there is a first-order correlation between the IR with the AME. Figure \ref{fig:AME_IR_crosscorr_allbandsg} visualizes the cross-correlation matrix for each of the IR bands. The AME does not show a strong correlation with other bands at $|l|>10$. At $|l|<10$, the FIR bands show stronger correlation. The AKARI 9 micron bands, and the other PAH-tracing bands show a stronger correlation only relative to the bands from 18 to 60 microns. The WISE 12 micron band is shown for comparison purposes, but the version of the WISE 12 micron map used here has been released only for visualization purposes \footnote{https://faun.rc.fas.harvard.edu/ameisner/wssa/index.html}.

    Even excluding pixels near the ecliptic plane, and separating the sky into two $l$ ranges, the situation is very difficult to interpret. To understand if the same pattern persists across the sky rather than presenting only for the all-sky average, we repeat the correlation analysis for $l$ bins. For each degree of $l$, we cross correlate that Planck modified blackbody fitting results vs. the AME and the AKARI 9 micron intensity. (This comes with the caveat that the galactic plane is much more heavily sampled than the galactic poles.) Figure \ref{fig:PlanckModBBvsAMEandA9_byGLAT} shows the results.
      ...
      \begin{figure*}
        ...
        \centering
        \caption{Cross-correlation tests (Spearman) of AME vs. dust radiance ($R$), temperature ($T$),  emissivity index (Beta), and the AKARI 9 micron brightness (A9), by $l$. The order of strongest to weakest correlation basically remains constant from pole to pole. (Except near the galactic plane, and $l = 40$  }
      \end{figure*}
