      ...
      A background estimation and subtraction is made
		...
		\subsection{Multi-wavelength characterization}
			...
			Contours indicate the region's shape in the PCAME map. Figure \ref{fig:orionis-corr} shows IR to AME cross correlation plots, for all pixels within the 10$^{\circ}$ by 10$^{\circ}$ $\lambda$~Orionis region.
                ...
                \end{figure}
            The correlation matrix results corresponding to data shown in \ref{{fig:orionis-corr}, are shown in \ref{fig:orionis-corr-matrix}
                \begin{figure}
            ...
            \subsubsection{Bootstrap analysis}
                ...
                We carry out bootstrap correlation tests for each IR band's intensity vs. the AME intensity. The data are resampled 10,000 times for each correlation test. This is repeated for both $r_{s}$ and $r_{p}$ based tests. The distributions of the boostrap resamplings are show in in Figs. \ref{fig:bootstrap_vs_AME}.
                    \begin{figure}
